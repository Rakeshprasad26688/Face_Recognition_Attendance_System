...
class Student:
    def __init__(self,root):
        ...
        title_label=Label(bg_image,text="STUDENT MANAGEMENT SYSTEM",font=("times new roman",35,"bold",),bg="red",fg='white')
        ...
        img_left = Image.open(r"D:\ty_project\college_images\Harebrained-Schemes4.jpg")
        img_left= img3.resize((720, 130), Image.ANTIALIAS)
        self.photoimg_left = ImageTk.PhotoImage(img_left)

        label_2 = Label(Left_frame, image=self.photoimg_left)
        label_2.place(x=5, y=0, width=720, height=130)

        #Current course
        Current_course_frame = LabelFrame(Left_frame, bd=2, bg="white", relief=RIDGE, text="Course Information",font=("time new roman", 12, "bold"))
        ...
        dep_combo['values']=("Select Department","BSC CS", "BSC IT","BCOM" )
        dep_combo.current(0)
        ...
        course_combo["values"] = ("Select Course", "Regular", "ATKT")
        ...
        semester_combo = ttk.Combobox(Current_course_frame,textvariable=self.var_semester,font=("time new roman", 12, "bold"), state="readonly",width=20)
        semester_combo["values"] = ("Select Semester", "Semester - 1", "Semester-2", "Smester-3", "Semester-4", "Semester-5", "Semester-6")
        ...
        student_name_label = Label(class_student_frame, text="Student Name:", font=("time new roman", 13, "bold"),
                                   bg="white")
        student_name_label.grid(row=0, column=2, padx=10, pady=5, sticky=W)

        student_name_entry = ttk.Entry(class_student_frame, textvariable=self.var_std_name, width=20,font=("time new roman", 13, "bold"))
        ...
        teacher_name_label = Label(class_student_frame, text="Teacher Name:", font=("time new roman", 12, "bold"),
                                   bg="white")
        teacher_name_label.grid(row=4, column=2, padx=10, pady=5, sticky=W)

        teacher_name_entry = ttk.Entry(class_student_frame, textvariable=self.var_teacher,  width=20,font=("time new roman", 12, "bold"))
        ...
        btn_frame.place(x=0, y=200, width=740, height=35)
        # save
        save_btn = Button(btn_frame, text="Save",command=self.add_data , width=18, font=("time new roman", 12, "bold"), bg="blue", fg="white")
        save_btn.grid(row=0, column=0)
        # update
        update_btn = Button(btn_frame, text="Update",command=self.update_data, width=18, font=("time new roman", 12, "bold"), bg="blue",fg="white")
        update_btn.grid(row=0, column=1)
        # delete
        delete_btn = Button(btn_frame, text="Delete",command=self.delete_data, width=18, font=("time new roman", 12, "bold"), bg="blue",fg="white")
        delete_btn.grid(row=0, column=2)
        # reset
        reset_btn = Button(btn_frame, text="Reset",command=self.reset_data, width=18, font=("time new roman", 12, "bold"), bg="blue", fg="white")
        ...
        btn_frame1.place(x=0, y=230, width=740, height=60)
        # take photo sample
        take_sample_btn = Button(btn_frame1,command=self.generate_dataset, text="Take Photo Sample", width=35, font=("time new roman", 12, "bold"),bg="blue", fg="white")
        take_sample_btn.grid(row=0, column=0)
        # update sample
        update_sample_btn = Button(btn_frame1, text="update Photo Sample", width=35,font=("time new roman", 12, "bold"), bg="blue", fg="white")
        ...
        img_2 = Image.open(r"D:\ty_project\college_images\2308522.jpg ")
        img_2 = img_2.resize((750, 130), Image.ANTIALIAS)
        self.photoimg_2 = ImageTk.PhotoImage(img_2)

        bg_image = Label(Right_frame, image=self.photoimg_2)
        bg_image.place(x=0, y=0, width=735, height=130) 

        # search system

        search_frame = LabelFrame(Right_frame, relief=RIDGE, text="Search System", font=("time new roman", 12, "bold"),
                                  bg="white")
        search_frame.place(x=5, y=135, width=730, height=70)

        search_label = Label(search_frame, text="SearchBy:", font=("time new roman", 15, "bold"), bg="white")
        ...
        "teacher", "photo"), xscrollcommand=scroll_x.set, yscroll=scroll_y.set)
    ...
    def add_data(self):
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            ...
            try:
                conn=mysql.connector.connect(host="localhost",username="root",password="Welcome$123",database="face_recognition")
    ...
    def reset_data(self):
        ...
        #================generate data for get a photo sample===================
    def generate_dataset(self):
        #photo_id=self.var_std_id.get()
        if self.var_dep.get() == "Select Department" or self.var_std_name.get() == "" or self.var_std_id.get() == "":
            messagebox.showerror("Error", "All Fields are required", parent=self.root)
            ...
            try:
                ...
                while True:
                    ...
                    if face_cropped(my_frame) is not None:
                        ...
                        file_name_path = "data/user." + str(id) +"."+str(img_id)+".jpg" #photo_id
